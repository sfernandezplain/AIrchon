...
[resources/sources-of-truth.md](resources/sources-of-truth.md) records
a live snapshot -- a git commit SHA for every `github.com` source, a
SHA-256 content hash for every other cited page -- of all 225 sources
across the five reference areas, generated by actually fetching each
one. That's the seed, not the sync: nothing yet re-runs this on a
schedule and diffs the result, so a page tagged **VERIFIED** is only as
current as the last time someone (or something) actually re-checked
it against that file. Generating it once already surfaced one dead
link and two malformed citations in the wiki-book itself -- real
findings a static page can't produce on its own.
