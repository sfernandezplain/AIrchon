...
.apm/skills/airchon-sync/                  Canonical source for a new, standalone, FORCED-only skill (added 2026-09-03 via /genesis) that closes the staleness-detection gap: re-checks every source in resources/sources-of-truth.json against its live current state (a fresh commit SHA for github.com sources via the bundled scripts/check_drift.py, a fresh SHA-256 hash for everything else), hands off to airchon-author per affected page with the specific evidence, then mechanically rebaselines sources-of-truth.md/.json (never hand-transcribed). Not routed through the airchon skill -- a maintenance/reconciliation operation doesn't fit that router's teach/author/mentor taxonomy. Confirmed live during design: bare-repo git citations (no file path) are NOT treated as actionable, since an active repo's HEAD commit changes constantly from unrelated work; content-hash checks on SPA-rendered doc sites (docs.github.com, platform.claude.com) carry a confirmed non-trivial false-positive rate from CDN cache cycling, not real content change -- see the skill's own SKILL.md for the full finding before scheduling this unattended.
...
references/rag/                            A RAG definitions/techniques reference (added 2026-08-24), sourced from the HuggingFace Cookbook notebooks and foundational papers -- author writes it, mentor and teacher read it, same as the wiki-book.
references/models/                         AI model classification for agentic development (added 2026-09-03) -- task/pipeline types, parameter-count scale, MoE/frankenmerging, quantization -- sourced from Hugging Face's own docs and named community writeups. Author writes it, mentor and teacher read it.
references/inference-engines/              Local/self-hosted inference-engine internals (added 2026-09-03) -- llama.cpp, Ollama, KTransformers -- split into engine-agnostic concept pages and per-engine implementation pages, cross-linking references/models/ for quantization and MoE rather than re-deriving them. Author writes it, mentor and teacher read it.
resources/sources-of-truth.md, .json       Live snapshot (added 2026-09-03) of every source cited across all five reference areas -- a git commit SHA per github.com source, a SHA-256 content hash per everything else, generated by actually fetching each one. The .json carries full unshortened values for a future diff script; the .md is the human-readable version. Not yet re-run on a schedule -- a one-time seed for the staleness-detection gap named in README.md, not an automated sync. Not owned by any single agent; regenerate by re-running the same extraction-and-fetch process against references/*/*.md's own citations.
