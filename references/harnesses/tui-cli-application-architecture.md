...
DeepSeek Harness (`dsh`, `github.com/deepseek-ai/deepseek-harness`, `master` branch, MIT license, TypeScript, developer preview) is, among the harnesses this page covers, the sole one that **does not ship a terminal UI at all**. Its README (fetched in full this session) describes exactly two interactive surfaces: `dsh web` (a browser application at `http://127.0.0.1:3080`) and `dsh --profile headless` (a one-shot stdout/stderr runner). Five bundle packages exist (`acp`, `base`, `headless`, `sdk-app`, `sdk-minimal`, `web-app`), every one of them VERIFIED from the repository tree listing this session, and not one contains a TUI framework, a terminal renderer, or a keybinding registry of its own. This section therefore documents what DeepSeek Harness actually has in this page's architectural domain -- a browser-only React application composed through a plugin-based Slots system on top of the Cordis framework, plus a headless runner with no rendering beyond process I/O -- and uses the absence itself as the architecturally significant finding it is: not every agentic coding tool solves the "many independently interactive terminal surfaces" problem this page traces, because not every one targets the terminal in the first place.

### 6.1 Rendering engine: a browser React application, not a terminal renderer

VERIFIED, `packages/bundle/web-app/README.md` (fetched in full this session): the `web` profile starts an HTTP server, prints a tokenized startup URL, and opens the default browser on it (unless `--no-open` is set or the invocation is over SSH). The browser exchanges that token for a signed session cookie and redirects to the clean root URL. There is no Ink, no OpenTUI, no blessed, no custom terminal renderer anywhere in the repository's `packages/` tree -- a recursive listing read this session confirms every client-facing package lives under `packages/client/` or `packages/bundle/web-app/`, and all are browser-side React/TypeScript, not terminal-oriented. The rendering engine is **React** itself: `packages/client/ui-renderer/README.md` (fetched in full this session) states plainly that `dsh-client-ui-renderer` "mounts the assembled dsh web client GUI" by calling `ctx.uiRenderer.mount(container)`, which "hydrates the framework-free boot page and switches to the full React application before the next paint." The boot kernel writes a composed `WebBootGraph` to `window.__DSH_BOOT__` before the page's scripts execute, the browser module loader is a lazy CommonJS table that registers and materializes plugin bundles, and after the complete client plugin roster settles the renderer performs the sole `renderSlot('root')` call that starts the React tree -- all VERIFIED from `docs/subsystems/web-client.md` and `docs/subsystems/client-modules.md` (both fetched in full this session). Layout is browser CSS, not Flexbox-on-a-terminal-grid: the Web Client's own layering table (web-client.md, fetched in full) names "Composition and rendering" as its topmost layer, powered by "React" explicitly, with no terminal-layout algorithm named anywhere. This is a categorical difference from every other section on this page: there is no alternate-screen buffer, no cell diffing, no synchronized-output escape sequence, no ANSI color negotiation, and no terminal-emulator capability-detection burden at all, because the rendering target is a browser DOM, not a character-cell grid.

The **headless** profile is the only surface that touches a terminal, and it is deliberately non-interactive. VERIFIED, `packages/bundle/headless/README.md` and `packages/bundle/headless/src/index.ts` (both fetched in full this session): `dsh --profile headless "run the tests"` creates one fresh Agent through the core registry, submits the task as a user message, streams non-empty provider reasoning deltas to stderr under a `dsh: reasoning:` heading, waits for quiescence, flushes the Session, and prints the final assistant text on stdout. There is no input prompt, no readline, no keybinding registry, no cursor management, and no alternate-screen buffer -- the runner's own `HeadlessIo` interface (`stdout: { write(chunk: string): unknown }`, `stderr: { write(chunk: string): unknown }`, `exit(code: number): void`) is the entire terminal I/O contract. The exit code maps the turn-end reason: 0 for `completed`, 1 for anything else (aborted, error, no turn in the owned interval). The reasoning stream, VERIFIED from source, groups consecutive reasoning deltas into one `dsh: reasoning:` section per reasoning block, closes the section before later output when the provider supplies no trailing newline, and keeps stderr empty for a successful run with no reasoning. This is architecturally closer to pi's own `print-mode.ts` (§4.3 -- "single-shot: send prompts, output result, exit") than to any of the interactive TUI surfaces §§1-5 document, but it is even more minimal than pi's print mode: where pi's print mode can emit a structured JSON event stream alongside plain text, the headless runner emits only human-readable text on stdout and progress-on-stderr, with no structured-output option.

```mermaid
flowchart LR
    subgraph CLI["dsh launcher"]
        Profile["--profile web/headless/sdk/acp/sdk-minimal"]
    end
    subgraph WebApp["web profile (browser)"]
        Host["Host Node process\n(Cordis + API controllers)"]
        Bootstrap["window.__DSH_BOOT__\nWebBootGraph"]
        ModLoader["Lazy CJS module table"]
        CordisBrowser["Cordis Loader (browser)"]
        Renderer["dsh-client-ui-renderer\nmount() -> React root"]
        Slots["ctx.slots.renderSlot('root')"]
    end
    subgraph HeadlessApp["headless profile (terminal)"]
        HeadlessRunner["headless-runner plugin\nAgent -> whenIdle() -> stdout"]
        Stdout["process.stdout\n(final text + newline)"]
        Stderr["process.stderr\n(reasoning deltas)"]
    end
    Profile --> Host
    Profile --> HeadlessRunner
    Host --> Bootstrap --> ModLoader --> CordisBrowser --> Renderer --> Slots
    HeadlessRunner --> Stdout
    HeadlessRunner --> Stderr
```

### 6.2 Component model: Cordis Slots -- a typed React composition system, not a terminal component tree

The Web Client's component architecture is architecturally unlike anything else on this page because it does not solve the "terminal grid component" problem at all -- it solves the browser-composition problem through a plugin-driven, typed React slot system built on the Cordis framework. VERIFIED, `docs/subsystems/slots.md` (fetched in full this session): `dsh-client-ui-slots` defines a "React-free registry and type algebra" (the compile-time `SlotMap`), while `dsh-client-ui-renderer` "binds observable sources to hooks, renders the tree, and owns React contexts internally." A feature plugin contributes UI through `ctx.slots.register()` and "never imports another feature plugin's component." This enforces a structural boundary this page's other harnesses do not need in the same way: in a terminal, components share a shared character grid and physical keyboard events; in the browser, each plugin's components are isolated React subtrees composed through the slot registry, with no shared rendering surface other than the DOM each slot outlet occupies.

The slot declaration model, VERIFIED from the same source, is a structured tree that reads as a genuine, detailed component hierarchy. Every slot has a **cardinality** (`single` -- one cell, priority winner renders; `list` -- ordered by `id`/`order`; `keyed` -- dispatched by an owner-supplied entry key; `chain` -- first non-null `select(owner)` result renders) and a **scope** (`root`, `session-maybe`, `session`) that determines which standard React hooks are available. The shipped hierarchy, quoted verbatim from the docs, is:
...
This is, by the standards of this page, the most explicit and complete component-tree specification of any harness covered -- not because it is more architecturally sophisticated than OpenCode's source-verified `dialog-*.tsx` hierarchy (§3.2), but because a browser application's layout tree is the kind of thing one documents in a Markdown hierarchy as a matter of course, while a terminal UI's component structure is usually left implicit in code. The hierarchy has direct structural parallels to the component trees this page infers or reads for the other harnesses: `conversation.composer` with its `input.*` and `overlay` subslots occupies the same architectural niche as Claude Code's `Chat` context (§1.2), Copilot CLI's main input area (§2.2), and OpenCode's prompt-input component (§3.2); `conversation.approval.detail` is the browser equivalent of the `Confirmation` contexts and `PermissionPrompt` components this page traces across §§1-5; `conversation.session.header.actions` mirrors the same header-actions pattern visible in Claude Code's `Footer` context and OpenCode's own toolbar bindings. The key architectural difference is that in a browser, all of this is composited from independently-loaded plugin bundles through the `SlotMap` type algebra and rendered into a DOM tree with full CSS layout, not accumulated into a character-cell grid through a reconciler or diffing engine.

**Conversation assembly** is a second, structurally distinctive layer, VERIFIED from `docs/subsystems/conversation.md` (fetched in full this session). `ui-conversation` binds to each `SessionBinding.eventSource` and runs an event-registry--view-registry pipeline: registered `ConversationNodeDefinition`s match standard events and packed Assistant runs by stable `(kind, id)`, fold deterministic state, and materialize target nodes; registered view definitions (`ui-chat`, `ui-trajectory`) build incremental target snapshots. This is the same "event-log -> assembled view" pipeline every harness on this page needs somewhere, but DeepSeek Harness documents it as an explicit, pluggable Conversation subsystem with an extension API (register your own Definitions and View Definitions) rather than leaving it as implementation detail inside a TUI component. The definition API (VERIFIED from the same source, including a complete worked example) requires each definition to declare `kind`, `target`, `match`, `start`, `update`, `publication`, `buildLocationData`, and `buildViewNode` -- a contract surface with no analogue in any terminal-UI harness on this page, where conversation-assembly logic is typically hardcoded inside the component that renders it.

**Approval (permission prompting) in the browser** acts through the same Cordis-event-driven architecture, not a modal dialog gate in a terminal event loop. VERIFIED, `docs/subsystems/approval.md` (fetched in full this session): the `ctx.approval` service accepts an `ApprovalRequest`, appends an `approval/asked` audit event, dispatches through an `approval/request` waterfall whose answerers may return an `ApprovalOutcome` (`allowed-once`, `rejected`, `cancelled`, `unavailable`), appends `approval/decided`, and resolves. The `approval/detail` slot in the conversation hierarchy is where a browser-side answerer renders the prompt -- architecturally the same role as Claude Code's `Confirmation` context (§1.2), Copilot CLI's three-option dialog (§2.3), and OpenCode's `PermissionPrompt` (§3.2), here implemented as a registered slot occupant in a DOM subtree rather than a terminal modal. The per-session `ApprovalPolicy` (`ask` or `never`) maps directly onto the same interactive-vs.-noninteractive decision every other harness's permission system makes (Claude Code's `Confirmation` context vs. `--dangerously-skip-permissions`, Copilot CLI's autopilot-mode suppression of dialogs, §2.2), but here the policy is an explicit per-session log event whose last value is authoritative, not a CLI flag or a runtime mode switch.

### 6.3 Input handling: the browser DOM is the input layer, not a terminal protocol

DeepSeek Harness's input handling is categorically different from every other harness on this page because the browser DOM, not a terminal emulator, reads and dispatches input events. There is no `Shift+Enter`/`Ctrl+J` terminal-protocol-negotiation problem (§§1.3, 2.3, 3.3, 4.3) because the browser's own input elements handle multi-line text natively; there is no Kitty-keyboard-protocol or `modifyOtherKeys` capability detection because the DOM reports modifier state reliably on every platform; there is no ANSI-escape-sequence keystroke parsing because keyboard events arrive as structured DOM `KeyboardEvent`s. The `conversation.composer` subtree's `input.*` subslots (§6.2) own the chat input area, but the actual text entry, paste handling, and cursor management are browser primitives, not a custom `Editor` or `textarea` terminal component like the ones this page documents for pi (§4.2) and OpenCode (§3.2). The `shell.overlay` slot at the root of the hierarchy is the closest analogue to the "modal overlay" concept the terminal harnesses need to implement manually (via `OverlayHandle` in pi, §4.2, or `<Portal>` in OpenCode, §3.3), but here it is simply a higher-z-index DOM element with standard browser focus management, no custom focus-arbitration API required.

The headless runner's input handling is the `task` CLI argument itself, VERIFIED from source: there is none after startup. The runner reads its task from `config.task` (populated by the `headless-startup` provider from the first positional argument parsed by `commander`), submits it as one user message, and never reads from stdin interactively. The `exitOnStdinEnd(ctx, label)` helper (VERIFIED from `packages/boot/cmdline/README.md`, fetched this session) binds stdin EOF to `ctx.appExit(0)` for stdio applications more broadly, but this is a teardown mechanism, not an input channel -- an idle headless run closes when its stdin pipe ends, not when it reads a keystroke.

**The architectural consequence for this page.** DeepSeek Harness's complete absence from the terminal-UI domain is not a gap in this page's coverage but a genuine data point: the "many independently interactive surfaces" problem this page traces across five terminal harnesses (chat prompt, permission dialog, diff viewer, session picker, each with its own keybinding vocabulary and focus state) is a terminal-UI-specific problem, and one that dissolves entirely when the rendering target is a browser, because the DOM carries its own input-event routing, focus management, modal dialog, and layout primitives that a terminal must recreate from scratch. The six-harness comparison below therefore treats DeepSeek Harness's row as a structural negative: no terminal renderer, no terminal component model, no terminal keybinding scoping mechanism -- not because the harness is incomplete, but because it chose a different rendering target that does not have the problem the other five sections are solving. The one problem domain that *does* carry over is the approval/permission prompt: every harness on this page needs some way to gate tool execution behind a user decision, and DeepSeek Harness does it through the same `approval/*` event + Composed-answerer pattern the terminal harnesses do through their permission-dialog components -- just rendered into a DOM slot instead of a character-cell overlay.
